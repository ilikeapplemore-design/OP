#!/usr/bin/env python3
# ==============================================================================
# agent_state.py – Version 1.0.3 (in‑place registry, never empties, forced report)
# ==============================================================================
...
def refresh_file_registry():
    ...
    Never clears the registry if the scan fails.
    """
    global _file_registry, _previous_file_set
    try:
        ...
        try:
            ...
            pass
        # still try to report current state
        if _file_registry:
            lines = [f"{fid}: {fname}" for fid, fname in sorted(_file_registry.items())]
            add_autonomous_report("files", "Files: " + " | ".join(lines))
